...
from utils import create_rllib_env, create_rllib_env_flatten
   ...
   # action_space = gym.spaces.MultiDiscrete([3, 3, 3])
    ...
    ppo = tune.run(
        ...
        config={
            ...
            "hiddens": [256],  # Hidden layers for DQN
    ...
    # Get the best trial based on max accuracy across all training iterations
    best_trial = ppo.get_best_trial("env_runners/episode_reward_mean", mode="max")
    print(best_trial)
    # Get the best checkpoint for the trial based on accuracy
...
def main():
    ...
    parser.add_argument('--use_lstm', default=False, action='store_true', help='Use LSTM policy model')
