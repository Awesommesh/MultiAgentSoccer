...
def train_ppo(args):
    ...
    striker_goalie = args.striker_goalie
    ...
    class SelfPlayUpdateCallback(DefaultCallbacks):
        ...
        def on_train_result(self, **info):
            ...
            """
            main_rew = info["result"]["env_runners"]["hist_stats"].pop("policy_default_striker_reward")
            won = 0
            total = 0
            for i in range(len(main_rew)):
                total += 1
                if main_rew[i] > 0:
                    won += 1
            win_rate = won / total
    ...
    def policy_mapping_fn(agent_id, *args, **kwargs):
        ...
        if agent_id < num_per_team:
            if agent_id % 2 == 0:
                return "default_striker"  # Choose 01 policy for agent_01
            else:
                return "default_goalie"
        else:
    ...
    ppo = tune.run(
        ...
        config={
            ...
            "multiagent": {
                "policies": {
                    "default_striker": (None, observation_space, action_space, {}),
                    "default_goalie": (None, observation_space, action_space, {}),
                ...
                "policies_to_train": ["default_striker", "default_goalie"],
            },
            "env": "Soccer",
...
def main():
    ...
    parser.add_argument('--striker_goalie', action='store_true', help='Train striker goalie specific agents (only works for 2v2)')
    ...
    parser.add_argument('--max_seq_len', type=int, default=10, help='LSTM max sequence length')
    ...
    parser.add_argument('--lstm_num_layers', type=int, default=1, help='LSTM number of layers')
